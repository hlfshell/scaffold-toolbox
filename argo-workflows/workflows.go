...
const quickStartManifest = "https://github.com/argoproj/argo-workflows/releases/latest/download/quick-start-minimal.yaml"
...
/*
Stack starts a Docker-backed k3s cluster, installs Argo Workflows, waits
for CRDs and controller rollout, and then applies user workflow manifests.
*/
type Stack struct {
...
// Option configures the Argo Workflows stack.
...
/*
WithWorkflowManifest registers a Workflow, WorkflowTemplate, or related
manifest to apply after Argo Workflows is installed and its CRDs are
available.
*/
func WithWorkflowManifest(path string) Option {
	return func(config *config) {
...
/*
WithOwnedCRDCleanup removes Argo Workflows CRDs on cleanup only if they
did not exist before this stack started. With beforeInstall set, those
owned CRDs are also cleared before install while preserving CRDs that
predated this run.
*/
...
/*
NewStack creates an Argo Workflows quickstart on a Docker-backed k3s
cluster.
*/
func NewStack(name string, options ...Option) (*Stack, error) {
	config := &config{namespace: "argo"}
...
func (s *Stack) Create(ctx context.Context) error {
	...
	if _, err := s.cluster.Kubectl(ctx, "apply", "--server-side", "-n", s.namespace, "-f", quickStartManifest); err != nil {
		s.cluster.Cleanup(context.WithoutCancel(ctx))
...
func (s *Stack) waitForCRDs(ctx context.Context) error {
	for _, name := range crdNames {
		if _, err := s.cluster.Kubectl(ctx, "wait", "--for", "condition=Established", "crd/"+name, "--timeout", "2m"); err != nil {
			return fmt.Errorf("argo workflows crd %s did not become established: %w", name, err)
		}
	}
